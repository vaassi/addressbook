...
async fn ldap_sync(State(state): State<AppState>) -> Result<Json<Value>> {
    ...
    let ldap_username =
        env::var(format!("LDAP_USERNAME")).expect("LDAP_USERNAME is not set in .env file");
    let ldap_password =
        env::var(format!("LDAP_PASSWORD")).expect("LDAP_PASSWORD is not set in .env file");
    let ldap_base_dn =
        env::var(format!("LDAP_BASE_DN")).expect("LDAP_BASE_DN is not set in .env file");
