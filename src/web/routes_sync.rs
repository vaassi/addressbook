...
async fn ldap_sync(State(state): State<AppState>) -> Result<Json<Value>> {
    // get env vars
    let ldap_urls = env::var("LDAP_URLS").expect("LDAP_URL is not set in .env file");
    let ldap_urls = ldap_urls.split(",").collect::<Vec<_>>();

    let birthdate_attr = env::var("BIRTHDATE_ATTR").expect("BIRTHDATE_ATTR is not set in .env file");
    let birthdate_fmt = env::var("BIRTHDATE_FMT").expect("BIRTHDATE_FMT is not set in .env file");

    // vec for data.json autocomplete
    let mut names = vec![];

    for (pos, ldap_url) in ldap_urls.iter().enumerate() {
        let ldap_username = env::var(format!("LDAP{}_USERNAME", pos+1)).expect("LDAP_USERNAME is not set in .env file");
        let ldap_password = env::var(format!("LDAP{}_PASSWORD", pos+1)).expect("LDAP_PASSWORD is not set in .env file");
        let ldap_base_dn = env::var(format!("LDAP{}_BASE_DN", pos+1)).expect("LDAP_BASE_DN is not set in .env file");
